...
If the user asked you to actually run a code review on a real codebase (not just supply review criteria), call `spawn_background_task(skill="com.mira.claudecode", brief="Review the code at <path>: …")` instead — that runs the `claude` CLI as a real reviewer subagent. This `review` tool only returns this prompt template; it does not read or analyze code itself.
