...
If the user asked you to actually BUILD something (not just plan), call `spawn_background_task(skill="com.mira.claudecode", brief="<full instruction>")` instead — that runs the `claude` CLI as a real coding subagent. This `plan` tool only returns this prompt template; it does not write or execute code itself.
