...
- **`wiki.auto_extract.auto_apply_above`** (number) — In mode='review', ops with extractor confidence at or above this threshold [0.0, 1.0] are applied immediately instead of waiting in the review queue; lower-confidence ops still land as pending. Omit/null (default) to require review for every extracted op. Ignored when mode is 'auto' or 'off'.
